...
from ..model.landuse_lutf_model import LUTFGeoModel


class LUTFGeoTrainer(BaseTrainer):

    def __init__(self, config):
        super(LUTFGeoTrainer, self).__init__(config)


    def build_model(self, device):

        self.model = LUTFGeoModel(
    ...
    def eval_step(self, batch, device):
        ...
        loss_ce, loss_g1, loss_g2, loss_g3 = self.model(x, x_dist, x_direct, x_degree, mx, comp)

        loss = loss_ce + (loss_g1 + loss_g2 + loss_g3) * self.config.train_config.loss_weight.geo_loss
